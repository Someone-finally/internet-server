...
char* get_wifi_connection_string(wifi_err_reason_t wifi_err_reason);
                   ...
                   int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
    {
        ...
        ESP_LOGW(TAG, "DISCONNECTED %d : %s",wifi_event_sta_disconnected->reason,get_wifi_connection_string(wifi_event_sta_disconnected->reason));
...
esp_err_t wifi_connect_sta(char *ssid, char *pass, int timeout)
{
    ...
    if (result == CONNECTED)
        return ESP_OK;             //Same as returning 0 
    return ESP_FAIL;               // Same as returning -1
} 
